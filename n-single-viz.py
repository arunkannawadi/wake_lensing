...
unit_length = 1.0
output_folder = "simulation_data_combined"  # Updated output folder
...
ax.plot([0, -unit_length], [0, -unit_length * np.tan(theta)], color='blue', linestyle='--')
ax.plot([0, -unit_length], [0, unit_length * np.tan(theta)], color='blue', linestyle='--')
...
ani = animation.FuncAnimation(fig, update, frames=max_frames, init_func=init, blit=True, interval=0.01)
